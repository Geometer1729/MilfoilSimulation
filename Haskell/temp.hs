...
main = do
  ...
  let dmap = map (\x -> x++[triInfrence dat x]) (grid [minx,minx+step..maxx] [miny,miny+step..maxy] )
  writeFile  "depth.txt"  (show dmap)
  hClose handle
...
cross (x1:x2:x3:[]) (y1:y2:y3:[]) = [x2*y3-x3*y2,x3*y1-y1*x3,x1*y2-x2*y2]
